...
import { getActiveSales, getAllSales, getExpenses, getCreditors, getPayments, Sale, Expense, Creditor, Payment } from '../../../lib/storage'
...
export default function TotalsPage() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      setSales(getActiveSales()) // Changed from getSales() to getActiveSales()
  ...
  const downloadPDF = async (): Promise<void> => {
    ...
    const allSales = getAllSales(); // This includes both active and deleted sales
    ...
    const totalCashSales = activeSales.filter(sale => !sale.isCredit).reduce((sum, sale) => sum + sale.amount, 0);
    const totalCreditSales = activeSales.filter(sale => sale.isCredit).reduce((sum, sale) => sum + sale.amount, 0);
    ...
    const comprehensiveHTML = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        ...
        <div style="margin-bottom: 30px;">
          ...
          <p><strong>Total Sales Records:</strong> ${allSales.length} (including ${deletedSalesCount} deleted entries)</p>
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 0.8rem;">
            ...
            <tbody>
              ${allSales.slice().reverse().map(sale => `
                <tr style="border-bottom: 1px solid #ddd; ${sale.deleted ? 'background-color: #ffebee;' : ''}">
                  <td style="padding: 8px; border: 1px solid #ddd; color: ${sale.deleted ? '#d32f2f' : '#388e3c'}; font-weight: bold;">
                    ${sale.deleted ? '❌ DELETED' : '✅ ACTIVE'}
                  </td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${new Date(sale.timestamp).toLocaleDateString()}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${sale.type}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${sale.itemName}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${sale.quantity}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${sale.amount.toFixed(2)} PKR</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${sale.paymentMethod}</td>
                  <td style="padding: 8px; border: 1px solid #ddd; color: ${sale.isCredit ? '#ffc107' : '#28a745'}">
                ...
                <tr style="background-color: #ffebee;">
                  <td colspan="8" style="padding: 5px 8px; border: 1px solid #ddd; color: #d32f2f; font-size: 0.7em;">
                    🗑️ Deleted on: ${sale.deletedAt ? new Date(sale.deletedAt).toLocaleString() : 'Unknown'}
                  </td>
                </tr>
                ` : ''}
              `).join('')}
            ...
            <tbody>
              ${allExpenses.slice().reverse().map(expense => `
                <tr style="border-bottom: 1px solid #ddd;">
                  <td style="padding: 8px; border: 1px solid #ddd;">${new Date(expense.timestamp).toLocaleDateString()}</td>
                  ...
                  <td style="padding: 8px; border: 1px solid #ddd;">${expense.amount.toFixed(2)} PKR</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 0.8rem;">
            ...
            <tbody>
              ${allCreditors.map(creditor => `
                <tr style="border-bottom: 1px solid #ddd;">
                  <td style="padding: 8px; border: 1px solid #ddd;">${creditor.name}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${creditor.phone}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${creditor.amountOwed.toFixed(2)} PKR</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${creditor.purchases.length} items</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 0.8rem;">
            ...
            <tbody>
              ${allPayments.slice().reverse().map(payment => `
                <tr style="border-bottom: 1px solid #ddd;">
                  <td style="padding: 8px; border: 1px solid #ddd;">${new Date(payment.timestamp).toLocaleDateString()}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${payment.creditorName}</td>
                  <td style="padding: 8px; border: 1px solid #ddd;">${payment.amount.toFixed(2)} PKR</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
    ...
    const opt = {
      margin: 10,
      filename: `cloudify_complete_business_report_${new Date().toISOString().slice(0, 10)}.pdf`,
    ...
    <div style={containerStyles}>
      ...
      <div id="totals-content">
        ...
        <div style={buttonContainerStyles}>
          ...
          <Link href="/totals/reports" style={reportsButtonStyles}> View Complete Reports</Link>
        </div>
      </div>
...
// (All the style objects remain unchanged)
